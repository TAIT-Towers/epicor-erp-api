...
class Task extends ServiceBase {
  ...
  async updateKey1(oldKey1, key2, relatedToFile, newKey1) {
    ...
    for (let t of dataset.Task) {
      t.RowMod = 'U';
      t.TEApprovalAction = 'App';
      t.Key1 = newKey1;
      // not sure why some tasks are created without a type code, but it is required by the API
      // removed TypeCode hack, as Ted figured out that by creating a new Labor TypeCode it
      // was now being defaulted on new records.  Leaving this comment here in case this bites us
      // in production!
