...
class ServiceBase {
  ...
  makeRequest(method, parameters, company) {
    return this.connection.makeRequest(this.serviceName, method, parameters, company)
  }
  ...
  create(record) {
    return this.makeRequest('GetNew' + this.datasetName, {
      ...
      .then(defaults => this.makeRequest('Update',
        {ds: {[this.datasetName]: [{...defaults, ...record, RowMod: 'A'}]}},
   ...
   */
  list(where, fields = '', {orderBy = '', limit = 100, listMethod = '', extraParams = {}} = {}) {
    if(!listMethod)
      listMethod = this.datasetName + (this.datasetName[this.datasetName.length - 1] === 's' ? '' : 's')
