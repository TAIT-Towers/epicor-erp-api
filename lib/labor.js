...
class Labor extends ServiceBase {
   ...
   */
  async updateLaborEntry(entry, customLaborHedData, customLaborDtlData) {
    new SimpleSchema({
      LaborHedSeq: Number,
    ...
    let ds = (
      await this.makeRequest('GetByID', {
    ...
    if (!dtl && entry.LaborDtlSeq)
      throw new Error(
        `LaborDtl id ${entry.LaborHedSeq} / ${entry.LaborDtlSeq} cannot be found`
      );
    ...
    if (customLaborHedData && !_compareHedData(hed, customLaborHedData)) {
      if (dtl) {
        ...
        recalledEntries = await this._recallEntriesDataset({
          LaborDtl: [dtl]
        });
        ...
        if (customLaborDtlData) Object.assign(dtl, customLaborDtlData);
  ...
  async updateRow(rowCriteria, newDtlValues) {
    ...
    if (dataset.LaborDtl.length) {
      ...
      if (isLaborTypeChanged) {
        //Hack fix to avoid DefaultLaborType overriding the first labordtl hours
        let laborHrs = 0;
        let burdenHrs = 0;
        if (dataset.LaborDtl[0].LaborHrs && dataset.LaborDtl[0].BurdenHrs) {
          laborHrs = dataset.LaborDtl[0].LaborHrs;
          burdenHrs = dataset.LaborDtl[0].BurdenHrs;
        }

        dataset = (
          await this.makeRequest('DefaultLaborType', {
            ds: dataset,
            ipLaborType: isIndirectCodeChanged ? 'I' : 'P'
          })
        ).parameters.ds;

        //After hours are overwritten, add them back
        if (dataset.LaborDtl[0].LaborHrs && dataset.LaborDtl[0].BurdenHrs) {
          dataset.LaborDtl[0].LaborHrs = laborHrs;
          dataset.LaborDtl[0].BurdenHrs = burdenHrs;
        }
      ...
      if (isIndirectCodeChanged) {
        dataset = (
          await this.makeRequest('DefaultIndirect', {
            ds: dataset,
      ...
      if (isProjectIDChanged) {
        dataset = (
          await this.makeRequest('DefaultProjectID', {
            ds: dataset,
      ...
      if (isJobNumChanged) {
        dataset = (
          await this.makeRequest('DefaultJobNum', {
            ds: dataset,
      ...
      if (isOprSeqChanged) {
        dataset = (
          await this.makeRequest('DefaultOprSeq', {
            ds: dataset,
  ...
  async _submitEntriesDataset(dataset) {
    ...
    ds.LaborDtl = dataset.LaborDtl.map(ld =>
      Object.assign(ld, {
        RowMod: 'U'
      })
    );
    ds = (
      await this.makeRequest('SubmitForApproval', {
        ds,
        lWeeklyView: false
      })
    ).parameters.ds;
  ...
  async _recallEntriesDataset(dataset) {
    ...
    ds = (
      await this.makeRequest('RecallFromApproval', {
        ds,
        lWeeklyView: false
      })
    ).parameters.ds;
  ...
  async _findOrCreateLaborHed(entry, customLaborHedData) {
    ...
    if (customLaborHedData) {
      where += Object.keys(customLaborHedData)
        .map(k => ` and ${k} = '${customLaborHedData[k]}'`)
        .join('');
    }
  ...
  async _createLaborDtl(entry, sourceDs, customLaborDtlData) {
    let workingDS = (
      await this.makeRequest('GetNewLaborDtl', {
        ds: {},
    ...
    if (entry.JobNum && entry.OprSeq !== undefined) {
      ...
      workingDS = (
        await this.makeRequest('DefaultAssemblySeq', {
          ds: workingDS,
          assemblySeq: 0
        })
      ).parameters.ds;
      ...
      workingDS = (
        await this.makeRequest('ChangeLaborType', {
          ds: workingDS
        })
      ).parameters.ds;
      workingDS = (
      ...
      workingDS = (
        await this.makeRequest('ChangeIndirectCode', {
          ds: workingDS
        })
      ).parameters.ds;
      workingDS = (
    ...
    Object.assign(workingDS.LaborDtl[0], {
      LaborHrs: entry.Hours,
      BurdenHrs: entry.IndirectCode ? 0 : entry.Hours,
      ReworkReasonCode: entry.ReworkCode ? entry.ReworkCode : '',
