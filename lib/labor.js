...
class Labor extends ServiceBase {
  ...
  async updateRow(rowCriteria, newDtlValues) {
    ...
    if (dataset.LaborDtl.length) {
      Object.keys(newDtlValues).forEach(
        key =>
          (typeof newDtlValues[key] === 'undefined' ||
            newDtlValues[key] == null) &&
          delete newDtlValues[key]
      );
      ...
      const isJobNumChanged = dataset.LaborDtl.some(
        ld => newDtlValues.JobNum && ld.JobNum !== newDtlValues.JobNum
      );
      const isOprSeqChanged = dataset.LaborDtl.some(
      ...
      if (isJobNumChanged) {
        ...
        changes.push('DisplayJob', 'JobType');
      }
      if (isOprSeqChanged) {
  ...
  async submitTimesheet(whereObject, datesToSubmit) {
    ...
    if (dataset.LaborDtl.length) {
      ...
      dataset = (await this.makeRequest('SubmitForApproval', {
        ds: dataset,
        lWeeklyView: false
      })).parameters.ds;
  ...
  async recallTimesheetEntries(entries) {
    ...
    if (dataset.LaborDtl.length) {
      dataset.LaborDtl = dataset.LaborDtl.filter(
        ld => !ld.NotSubmitted && !ld.PostedToGL
      ).map(ld => Object.assign(ld, {RowMod: 'U'}));
      dataset = (await this.makeRequest('RecallFromApproval', {
        ds: dataset,
        lWeeklyView: false
      })).parameters.ds;
      dataset = (await this.makeRequest('Update', {ds: dataset})).parameters.ds;
