...
class Labor extends ServiceBase {
  ...
  async deleteLaborEntry(entry) {
    ...
    const {
      dataset
    } = await this._getRows(`LaborHedSeq='${LaborHedSeq}'`);
  ...
  async updateRow(rowCriteria, newDtlValues) {
    ...
    let {
      dataset
    } = await this._getRows(where, 100, 0, whereDtl.join(' and '));
    ...
    if (dataset.LaborDtl.length) {
      ...
      if (isIndirectCodeChanged) {
        ...
        dataset.LaborDtl.forEach(ld => Object.assign(ld, {
          ProjectID: ''
        }));
      }
  ...
  async recallTimesheetEntries(entries) {
    ...
    let {
      dataset
    } = await this._getRows(where, 100, 0, whereDtl);
    if (dataset.LaborDtl.length) {
  ...
  async _submitEntriesDataset(dataset) {
    ...
    ds.LaborDtl = dataset.LaborDtl.map(ld => Object.assign(ld, {
      RowMod: 'U'
    }));
  ...
  async _findOrCreateLaborHed(entry, customLaborHedData) {
    ...
    let {
      dataset
    } = await this._getRows(where, 50, 0);
  ...
  async _createLaborHed(entry, customLaborHedData) {
    const {
      parameters
    } = await this.makeRequest('GetNewLaborHed1', {
      ds: {},
