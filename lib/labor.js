...
class Labor extends ServiceBase {
  ...
  async deleteLaborEntry(entry) {
    ...
    if(entry.LaborDtlSeq <= 0) {
      ...
      const dtl = dataset.LaborDtl.filter(d => d.LaborDtlSeq === entry.LaborDtlSeq)[0]
      if(dtl) {
        dtl.RowMod = 'D'
  ...
  async deleteRow(rowCriteria) {
    ...
    if(dataset.LaborDtl.length) {
      ...
      const hedSeqs = Object.keys(R.indexBy(R.prop('LaborHedSeq'), dataset.LaborDtl))
   ...
   *
   * @param whereObject Object  FromDate, ToDate and EmployeeNum to use to pull the timesheet
   * @param activeJobs  [Object]  Optional set of jobs that are currently active.
   *      If this is specified, only the jobs that are present in that map will be included in the recall
   */
  async recallTimesheet(whereObject, {activeJobs} = {}) {
    new SimpleSchema({
      FromDate: /\d{4}-\d{2}-\d{2}/,
      ToDate: /\d{4}-\d{2}-\d{2}/,
      EmployeeNum: String,
    ...
    let {dataset} = await this._getRows(where, 100, 0)
    if(dataset.LaborDtl.length) {
      dataset.LaborDtl = dataset.LaborDtl
        .filter(ld => (!ld.NotSubmitted && !ld.PostedToGL) &&
          // only the ones that are active, if we were given an activeJobs parameter
          !(ld.JobNum && activeJobs && !activeJobs[ld.JobNum]))
  ...
  async _createLaborDtl(entry, sourceDs, customLaborDtlData) {
    ...
    if(entry.ProjectID) {
      workingDS = (await this.makeRequest('DefaultProjectID', { ds: workingDS, ipProjectID: entry.ProjectID })).parameters.ds
    }
    if(entry.JobNum && entry.OprSeq !== undefined) {
      workingDS = (await this.makeRequest('DefaultJobNum', { ds: workingDS, jobNum: entry.JobNum })).parameters.ds
      workingDS = (await this.makeRequest('DefaultAssemblySeq', { ds: workingDS, assemblySeq: 0 })).parameters.ds
      workingDS = (await this.makeRequest('DefaultOprSeq', { ds: workingDS, oprSeq: entry.OprSeq })).parameters.ds
    } else if(entry.IndirectCode) {
      workingDS = (await this.makeRequest('ChangeLaborType', { ds: workingDS })).parameters.ds
      workingDS = (await this.makeRequest('DefaultLaborType', { ds: workingDS, ipLaborType: 'I' })).parameters.ds
      workingDS = (await this.makeRequest('ChangeIndirectCode', { ds: workingDS })).parameters.ds
      workingDS = (await this.makeRequest('DefaultIndirect', { ds: workingDS, indirectCode: entry.IndirectCode })).parameters.ds
    } else {
  ...
  async _recalculateLaborHedTotals(laborHedSeq, deleteEmptyHed = false) {
    const where = Array.isArray(laborHedSeq) ? `LaborHedSeq in (${laborHedSeq.join(',')})` : `LaborHedSeq = ${laborHedSeq}`
...
function _recalculateLaborHedTotalsInDataset(dataset, deleteEmptyHed = false) {
  ...
  for(let hed of dataset.LaborHed) {
    ...
    for(let dtl of dataset.LaborDtl) {
      if(dtl.LaborHedSeq === hed.LaborHedSeq) {
        payHours += dtl.LaborHrs
      }
    }
    // not sure if we should remove them if payHours reaches 0?
    if(payHours > 0 && hed.PayHours !== payHours) {
      hed.PayHours = payHours
      hed.RowMod = 'U'
      wasChanged = true
    }
    if(payHours === 0 && deleteEmptyHed) {
      hed.RowMod = 'D'
      wasChanged = true
    }
  }
...
function _validateForSubmission(dataset) {
  const timesPerDay = {}
  dataset.LaborDtl.forEach(ld => {
    timesPerDay[ld.PayrollDate] = (timesPerDay[ld.PayrollDate] || 0) + ld.LaborHrs
  })
