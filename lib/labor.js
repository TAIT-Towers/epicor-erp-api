...
class Labor extends ServiceBase {
  ...
  /**
   * Update an existing LaborDetail object.
   *
   * If customLaborHedData is provided, we'll check to make sure it matches the existing LaborHed record, if it doesn't,
   * we'll delete the previous entry and create a new one with that data.
   */
  async updateLaborDetail(entry, customLaborHedData) {
    new SimpleSchema({
      LaborHedSeq: Number,
      LaborDtlSeq: Number,
      Hours: Number,
    ...
    if(customLaborHedData && !_compareHedData(hed, customLaborHedData)) {
      ...
      // we need to:
      // - create new header
      // - create entry under that header
      // - delete old entry
      // - if old header is empty, delete it
   ...
   */
  async deleteLaborDetail(entry) {
    new SimpleSchema({
      LaborHedSeq: Number,
      LaborDtlSeq: Number
    }).validate(entry, {ignore: SimpleSchema.ErrorTypes.KEY_NOT_IN_SCHEMA})
    const {LaborHedSeq, LaborDtlSeq} = entry
    const {dataset} = await this._getRows(`LaborHedSeq='${LaborHedSeq}'`, 100, 0)
    dataset.LaborDtl.forEach(dtl => {
      if(dtl.LaborDtlSeq === LaborDtlSeq)
        dtl.RowMod = 'D'
    })
    if(dataset.LaborDtl.length === 1 && dataset.LaborDtl[0].RowMod === 'D') {
      dataset.LaborHed[0].RowMod = 'D'
      // the hours on the LaborHed record will be updated automatically
    }
  ...
  async submitTimesheet(whereObject) {
    ...
    const {FromDate, ToDate, EmployeeNum} = whereObject
