...
class Labor extends ServiceBase {
  ...
  async updateLaborEntry(entry, customLaborHedData, customLaborDtlData) {
    ...
    // check if labordtl data has changed
    if (dtl && customLaborDtlData && !_compareDtlData(dtl, customLaborDtlData)) {
      Object.assign(dtl, customLaborDtlData)
    }
  ...
  async _createLaborDtl(entry, sourceDs, customLaborDtlData) {
    ...
    if (entry.JobNum && entry.OprSeq !== undefined) {
      ...
      workingDS = (await this.makeRequest('ChangeLaborType', {
        ds: workingDS
      })).parameters.ds;
...
function _buildWhereRow(rowCriteria) {
  ...
  if (ReworkReasonCode)
    whereDtl.push(`ReworkReasonCode = '${ReworkReasonCode}'`);
...
function _compareDtlData(dtl, newDtlData) {
  for (let k in newDtlData) {
    if (dtl[k] !== newDtlData[k]) return false;
  }
  return true;
}
...
function _findDetailEntry(dataset, entry) {
  const jobNum = entry.JobNum || '',
    oprSeq = entry.OprSeq || '',
    indirectCode = entry.IndirectCode || '',
    reworkCode = entry.ReworkCode || '';
  return R.find(
    j =>
      j.JobNum === jobNum &&
      j.OprSeq === oprSeq &&
      j.IndirectCode === indirectCode &&
      j.ReworkCode === reworkCode,
    dataset.LaborDtl
  );
}
