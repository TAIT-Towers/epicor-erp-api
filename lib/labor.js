...
class Labor extends ServiceBase {
  ...
  async submitTimesheet(whereObject, datesToSubmit) {
    ...
    if(dataset.LaborDtl.length) {
      dataset.LaborDtl.forEach(ld => {
        ...
        if(ld.NotSubmitted && datesToSubmit.includes(payrollDate)) {
          Object.assign(ld, {
            EnableSubmit: true,
            TimeAutoSubmit: true,
            RowMod: 'U'
      ...
      dataset.LaborDtl.forEach(ld => {
        ...
        if (datesToSubmit.includes(payrollDate)) {
          Object.assign(ld, { RowMod: 'U' });
        }
      });
  ...
  async recallTimesheetEntries(entries) {
    const where = `LaborHedSeq in (${R.uniq(R.map(R.prop('LaborHedSeq'), entries)).join(',')})`
    const whereDtl = `LaborDtlSeq in (${R.map(R.prop('LaborDtlSeq'), entries).join(',')})`
...
function _findDetailEntry(dataset, entry) {
  ...
  return R.find(j => j.JobNum === jobNum && j.OprSeq === oprSeq && j.IndirectCode === indirectCode,
    dataset.LaborDtl)
}
