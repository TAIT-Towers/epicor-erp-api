...
class Labor extends ServiceBase {
  ...
  async deleteRow(rowCriteria) {
    ...
    if(dataset.LaborDtl.length) {
      ...
      const hedSeqs = dataset.LaborHed.map(h => h.LaborHedSeq)
      await this.makeRequest('Update', {ds: dataset})
      await this._recalculateLaborHedTotals(hedSeqs)
      return true
    }
    return false
  }
  ...
  async _findOrCreateLaborHed(entry, customLaborHedData) {
    ...
    console.log('_findOrCreateLaborHed, got dataset - hed', dataset.LaborHed.length)
    console.log('_findOrCreateLaborHed, got dataset - dtl', dataset.LaborDtl.length)
