...
describe('ServiceBase', () => {
  describe('find', () => {
    ...
    it('processes multiple pages when morePage is true', done => {
      ...
      result.on('end', () => {
        data.should.eql([{id: 1}, {id: 2}]);
        expect(fakeRequest).to.have.been.calledWith(
          'GetList',
          sinon.match({absolutePage: 0})
        );
        expect(fakeRequest).to.have.been.calledWith(
          'GetList',
          sinon.match({absolutePage: 1})
        );
        done();
      });
    });
    ...
    it('calls GetList with where clause', done => {
      service.makeRequest = sinon.stub().returns(
        Promise.resolve({
          parameters: {},
          returnObj: {PropertyList: []}
        })
      ...
      service.makeRequest = sinon.stub().returns(
        Promise.resolve({
          ...
          returnObj: {
            ...
            Children: [{Company: 150, ChildId: '2', ParentId: '2'}]
          }
        })
      );
      const records = [];
